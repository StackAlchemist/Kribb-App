...
const PRICE_PRESETS = [
    { label: "Under ₦50M", min: null, max: 50000000 },
    { label: "₦50M – ₦100M", min: 50000000, max: 100000000 },
    { label: "₦100M – ₦250M", min: 100000000, max: 250000000 },
    { label: "Above ₦250M", min: 250000000, max: null },
  ];
    ...
    >
      <View className="flex-1 bg-gray-50 ">
          ...
          <View className="flex-row flex-wrap gap-2 mb-6">
            {BEDS.map((item)=>(
              <TouchableOpacity
                ...
                style={shadow}
              >
                <Text className={`text-sm font-bold ${chipText( bedrooms === item.value)}`}>
                  {item.label}
                </Text>
              </TouchableOpacity>
            ))}
            ...
            {PRICE_PRESETS.map((p)=>{
              ...
              return (
                <TouchableOpacity
                  ...
                  style={shadow}
                >
                  <Text className={`text-xs font-medium ${active ? "text-blue-600" : "text-gray-500"}`}>
                    {p.label}
                  </Text>
                </TouchableOpacity>
              )
        ...
        <View className="px-5 pb-8 pt-4 bg-white border-t border-gray-100">
          ...
          >
            <Text className="text-white font-bold text-base">Apply Filters {activeCount > 0 ? `(${activeCount})`: ""}</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
